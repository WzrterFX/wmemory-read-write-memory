...
#define adr64 const uint64
#define adr32 const uint32

class Wmemory
{
public:
    Wmemory(const wchar_t* name)
    {
    ...
    uint64 GetModuleBase(const wchar_t* name)
    {
        uint64 base = NULL;
        ...
        if (Module32FirstW(snapshot, &module))
        {
            do
            {
                ...
                {
                    base = reinterpret_cast<uint64>(module.modBaseAddr);
    ...
    template <typename T>
    void WriteMemory(uint64 address, const T& value)
    {
